...
import 'package:to_do_app/features/note/domain/usecases/note_use_case/get_all_notes.dart';
import 'package:to_do_app/features/note/domain/usecases/note_use_case/get_note_by_user_id.dart';
...
void main() {
  ...
  setUp(() {
    ...
    getNoteByUserIDUseCase = GetNoteByUserIDUseCase(mockNoteRepository);
  ...
  group('Get Notes', () {
    ...
    test('shuold get correct list notes from the current user', () async {
      ...
      // act
      final result =
          await getNoteByUserIDUseCase(const GetNoteParams(userId: userId));
      // assert
      expect(result, Right(tNotes));
